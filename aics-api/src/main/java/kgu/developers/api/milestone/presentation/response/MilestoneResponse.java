...
public record MilestoneResponse(
    ...
    @Schema(description = "마일스톤 ID", example = "15", requiredMode = REQUIRED)
    Long id,

    @Schema(description = "분반 ID", example = "1", requiredMode = REQUIRED)
    Long sectionId,

    @Schema(description = "마일스톤 제목", example = "발표", requiredMode = REQUIRED)
    String title,

    @Schema(description = "마일스톤 설명")
    String description,

    @Schema(description = "진행 주차", example = "12", requiredMode = REQUIRED)
    int weekNumber,
    ...
    @Schema(
        description = "마일스톤 유형. PRESENTATION은 발표자료 제출과 발표 평가에 함께 사용한다.",
