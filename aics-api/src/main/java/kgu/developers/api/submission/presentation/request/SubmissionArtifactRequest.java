...
// LINK/TEXT/CHEERPJ_RUN 아티팩트 전용(파일은 별도 files 파트 + fileArtifactIds로 correlate).
public record SubmissionArtifactRequest(
        ...
        @Schema(description = "아티팩트 종류(LINK, TEXT, CHEERPJ_RUN)", example = "LINK", requiredMode = REQUIRED)
        @NotNull
        ArtifactType type,

        @Schema(description = "LINK 또는 CHEERPJ_RUN일 때의 URL", example = "https://github.com/kgu-developers/aics-team-project-server")
        String url,
