...
public class ImportBatchRepositoryImpl implements ImportBatchRepository {
	...
	@Override
	public Optional<ImportBatch> findLatestAppliedBySectionIdAndType(Long sectionId, Type type) {
		return jpaImportBatchRepository
				.findFirstBySectionIdAndTypeAndStatusAndDeletedAtIsNullOrderByUpdatedAtDesc(sectionId, type,
						Status.APPLIED)
				.map(ImportBatchJpaEntity::toDomain);
	}
}
